...
`;
		...
		*/
		...
		function apply(ctx) {
			...
			const connectWorkspace = (workspaceId) => {
				const workspace = ctx.workspaces.list.getSnapshot().items.find((item) => item.workspaceId === workspaceId);
				if (workspace === void 0) return Promise.reject(/* @__PURE__ */ new Error(`ya-workspace-sidebar: unknown workspace "${workspaceId}"`));
			...
			const startSession = (workspaceId) => {
				...
				connectWorkspace(workspaceId).then((sessionId) => {
