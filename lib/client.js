...
`;
		...
		*/
		function deriveWorkspaceSessionGroups(key, list, workspaces, archivedSessionIds, now) {
			...
			if (workspace === void 0) return [];
			const archived = new Set(archivedSessionIds);
			return groupByLocalDate(workspace.sessionIds.map((id) => list.byId[id]).filter((summary) => summary !== void 0 && visible(summary, list.current, archived)).map((summary) => rowOf(summary, workspace.workspaceId, workspace.title)), (row) => row.updatedAt, (a, b) => b.updatedAt - a.updatedAt || String(a.id).localeCompare(String(b.id)), now);
		}
